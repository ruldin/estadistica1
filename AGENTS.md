...
Este proyecto es una **plataforma educativa estática de apoyo docente** desarrollada por el profesor **Ruldin Ayala** para los cursos de **Estadística 1** e **Informática I** de la carrera de **Administración de Empresas** en la **Universidad Mariano Gálvez de Guatemala (UMG)**. Cada materia atiende a **secciones/estudiantes distintos**, por lo que cada portal (`Estadistica1/`, `informatica1/`) opera de forma independiente aunque comparte la misma infraestructura de configuración IA.
...
- [Estadistica1/index_estadistica.html](file:///d:/U/2026/Semestre2/Estadistica/WebAppEstadistica/Estadistica1/index_estadistica.html): Página principal / Dashboard de Estadística I con accesos directos por semana, logo UMG, navegación y **Centro de Configuración de API OpenRouter para el Alumno** (gestión local y segura en `localStorage` de `bias-lab-settings` con modelo por defecto `inclusionai/ling-3.0-flash-fin:free`, selector de modelos, botón de prueba en vivo y purga de seguridad).
- [informatica1/index_informatica.html](file:///d:/U/2026/Semestre2/Estadistica/WebAppEstadistica/informatica1/index_informatica.html): Página principal / Dashboard del curso de **Informática I** con diseño visual adaptado (paleta cian/azul/índigo), accesos directos a herramientas de IA, mapa de contenidos, enlace al Módulo de Semana 7 y **Centro de Configuración de API OpenRouter para el Alumno** (mismo sistema que Estadística I: botón en header con badge de estado, panel "Servicio de IA para el Estudiante" con resumen de modelo/seguridad/estado, modal `api-config-modal` con guía paso a paso, selector de modelos, prueba de conexión en vivo y purga de seguridad; claves `localStorage` compartidas `bias-lab-settings` / `openrouter_settings` con modelo por defecto `inclusionai/ling-3.0-flash-fin:free`, por lo que la llave configurada en un portal funciona automáticamente en el otro).
...
- [informatica1/Semana9/Semana9Informatica.html](file:///d:/U/2026/Semestre2/Estadistica/WebAppEstadistica/informatica1/Semana9/Semana9Informatica.html): Semana 9 de Informática I — Redes y Telecomunicaciones GT: LAN vs. WAN, Medios de transmisión, Simulador "Arma tu Red", Caso Farmacia Nebaj, Ejercicio en parejas y autoevaluación con diseño en tema oscuro ejecutivo.
...
- [Estadistica1/sem7_frecuencia_pareto.html](file:///d:/U/2026/Semestre2/Estadistica/WebAppEstadistica/Estadistica1/sem7_frecuencia_pareto.html): Semana 7 — Polígonos de Frecuencia, Ojivas e Histogramas Comparativos: tutorial paso a paso para gráficos cuantitativos continuos (Histograma, Polígono en marcas de clase Xi, Ojivas menor que y mayor que), superposición mixta (Histograma + Polígono) y superposición comparativa (Serie A vs Serie B) para contrastar turnos y sucursales (caso Textiles San Lucas S.A.), etiquetado personalizado de series, desglose paso a paso colapsable/expandible de todas las columnas (fi, fr, fp%, fp acum%, Fa, Fa Mayor, Xi), cálculo conjunto estandarizado en modo comparativo usando la muestra total ($N = n_A + n_B$, rango global $[X_{\min}, X_{\max}]$ e intervalos idénticos para permitir superposición válida), selector flexible del número de clases (Regla de Sturges por defecto vs. Definición Manual/Libre de $k$ con actualización dinámica de comentarios en tutorial paso a paso), laboratorio interactivo con soporte de unidades de tiempo (horas, minutos, segundos), y exportación de informe ejecutivo a PDF (jsPDF).
- [Estadistica1/sem8_medidas_tendencia_central.html](file:///d:/U/2026/Semestre2/Estadistica/WebAppEstadistica/Estadistica1/sem8_medidas_tendencia_central.html): Semana 8 — Medidas de Tendencia Central (Media, Mediana y Moda para la toma de decisiones): Guía conceptual gerencial (Parámetro vs Estadístico, la Media como centro de gravedad y vulnerabilidad ante outliers 'elefantes', la Mediana como topografía inmune y reina de salarios/esperas, la Moda en el retail guatemalteco y bimodalidad de mercados segmentados, sesgo positivo/negativo y dilema ético con algoritmos de IA); Caso interactivo del Gran Duelo Gerencial (Puesto tradicional de Garnachas en la Feria Patronal con el dilema entre compra de insumos diarios y solicitud de crédito bancario); Laboratorio con 6 casos predefinidos, selector de tipos de datos, selector interactivo de vistas gráficas (Histograma, Polígono de Frecuencias en marcas de clase Xi y Ojiva Acumulada), ingreso libre de dataset, botón de limpiar, cálculo automático de KPIs con diagnóstico ejecutivo de decisiones, tabla de distribución de frecuencias, exportación de informe en PDF (jsPDF), y **Laboratorio IA interactivo integrado en iframe** (`https://temporary-instant-apogee-hran4bx.vercel.app/`) para auditar en 5 pasos el impacto del System Prompt y la elección entre Media y Mediana en decisiones de agentes de IA.
- [Estadistica1/sem8_ejercicio.html](file:///d:/U/2026/Semestre2/Estadistica/WebAppEstadistica/Estadistica1/sem8_ejercicio.html): Semana 8 — CentralTrend (Ejercicios y Casos Prácticos): Plataforma gamificada interactiva para resolución de 10 casos de negocio reales aplicando Media, Mediana y Moda (retail, logística en Calzada Roosevelt, salarios en Call Center Zona 10, métodos de cobro en La Terminal Z.4, cafeterías en Cobán y agroexportación de café en Sacatepéquez), con diseño oscuro ejecutivo, encabezado institucional UMG, **modal de Evaluación IA formativa conectada a OpenRouter con tono de Mentor Amigo** (tratamiento cercano de "tú", constructivo, empático y alentador sin calificativos punitivos ni saludos iniciales redundantes, comenzando directamente con el contenido pedagógico), panel de reevaluación en **recuadro tipo acordeón colapsado por defecto** que permite probar puntualmente otros modelos de IA sin modificar ni sobreescribir la configuración principal del alumno en `localStorage`, sanitización estricta de trazas de razonamiento/`think`, e impresión en el reporte PDF del análisis del alumno, respuesta de la IA y criterio técnico oficial.
- [Estadistica1/sem9_agrupados.html](file:///d:/U/2026/Semestre2/Estadistica/WebAppEstadistica/Estadistica1/sem9_agrupados.html): Semana 9 — Estadígrafos de Tendencia Central para Datos Agrupados y Medidas de Posición: Guía interactiva integral con **selector visual destacado de 4 Casos de Negocio Gerenciales** cargados dinámicamente desde [Estadistica1/contexto_sem9_agrupado.md](file:///d:/U/2026/Semestre2/Estadistica/WebAppEstadistica/Estadistica1/contexto_sem9_agrupado.md) (Caso 1: Microcréditos Cooperativa Pinulteco en Q, n=50; Caso 2: Salarios Call Center Zona 10 en Q, n=56; Caso 3: Despacho Farmacia Roosevelt en min, n=50; Caso 4: Pruebas de Aptitud Admisiones UMG en pts, n=30), cambio dinámico integral del Banner de Briefing Ejecutivo (categoría, ubicación, badge de muestra `briefing-badge-n`, contexto del problema y las 3 fichas de indicadores urgentes contextualizadas con sus títulos y preguntas gerenciales), conmutación de estado a `[Activado]`, limpieza automática de cálculos previos, conclusiones y dictámenes de IA (`clearStudentWork`) al cambiar de caso o pulsar "Cargar enunciado a mi Ficha", actualización automática de la Ficha del Estudiante; **navegación guiada paso a paso** con botones de Anterior y Siguiente entre los 7 pasos; **asistente formativo de Inteligencia Artificial (Mentor Amigo)** con botón de retroalimentación en cada paso y evaluación del pensamiento crítico conectado a OpenRouter API (`inclusionai/ling-3.0-flash-fin:free`, selector de modelos, modal de configuración con almacenamiento local `bias-lab-settings`/`openrouter_settings` y purga de seguridad); tabla fuente sticky en desktop y modal flotante en móviles; cálculo completo de Media ponderada, Mediana con Fi ordinal, Moda con interpolación Δ₁/Δ₂, y Cuartiles/Deciles/Percentiles; histograma + ojiva interactiva; simulador gerencial de políticas de cartera y **generador de Hoja de Trabajo oficial en PDF (jsPDF)** que imprime de forma estructurada tanto los análisis y cálculos del estudiante como los dictámenes pedagógicos de la IA para cada paso y para las conclusiones generales.
- [Estadistica1/contexto_sem9_agrupado.md](file:///d:/U/2026/Semestre2/Estadistica/WebAppEstadistica/Estadistica1/contexto_sem9_agrupado.md): Catálogo estructurado de casos empresariales de Semana 9 para datos agrupados con enunciados, preguntas clave, contexto guatemalteco, prompts para IA y directrices pedagógicas.
- [Estadistica1/casos.txt](file:///d:/U/2026/Semestre2/Estadistica/WebAppEstadistica/Estadistica1/casos.txt): Fuente de casos y datasets crudos para ejercicios de medidas descriptivas.
...
- `vendor/`: Librerías JS empaquetadas localmente (Chart.js + plugin datalabels) usadas por Semanas 4, 5, 6, 7 y 8.
...
7. Semana 8 — Medidas de Tendencia Central: Media, Mediana y Moda (+ Duelo Garnachas + Laboratorio Gráficos/PDF + Laboratorio IA Integrado)
8. Semana 9 — Estadígrafos de Tendencia Central para Datos Agrupados y Medidas de Posición (+ 4 Casos Dinámicos + Tutor Mentor IA + Simulador de Políticas + PDF)
...
2. Semana 9 — Redes y Telecomunicaciones GT en `informatica1/Semana9/` (HTML + `styles.css` + `app.js` con ejercicio en parejas y PDF vía impresión; logo UMG en navbar vía `logo.png`).
3. Semana 10 — Ciberseguridad e Integridad de la Información en `informatica1/Semana10/` (`Semana10Ciberseguridad.html` + `styles.css` + `app.js` con logo UMG en navbar, PDF directo con jsPDF + autotable vía CDN —compatible con teléfonos—, identificación de pareja sin jornada; imágenes CC en `img/`; **tarea individual "La Silla del Gerente"** (sección `#tarea`: caso adaptado Café Las Nubes Cobán —rescate Q36,000/72h, pérdida Q5,200/día—, decisión + 3 argumentos + cálculos con operaciones + regla PUA + conclusión, botón Verificar con tutoría sin regalar la nota, PDF individual para Canvas); fuente del caso Sección B del parcial en `fuentes_seccionb.txt` (Lácteos El Progreso, Teculután); **mentoría IA por caso en el ejercicio en parejas**: panel "Mentoría IA · Mentor Amigo" con badge de estado, consulta proactiva automática tras Guardar, modal de retroalimentación con punto de vista + retroalimentación técnica + consejo gerencial, prompts compactos con truncado (`trunc`, `max_tokens: 1200` (alineado con Estadística; reintento único a 2000 si `finish_reason=length`), `temperature: 0.4`) para cuidar tokens de contexto, exclusión de razonamiento en la petición (`reasoning: {exclude: true}`) más system prompt que prohíbe mostrar el proceso de análisis, sanitización de `<think>` y de preámbulos de cadena de pensamiento ("Analyze the Request" / "Evaluate the Student's Response", recorte hasta el primer apartado real), system prompt 100% en español sin ejemplos en inglés (para evitar eco de instrucciones), filtro de líneas de evaluación interna (`-> Correct/Partially`, campos `Goal/Method/Justification`) y descarte con mensaje de reintento/cambio de modelo si no queda ningún apartado real, gating suave que NUNCA bloquea —si no hay key o falla la conexión el alumno continúa—, borrador automático al navegar (`guardarBorrador`/`avanzarCaso`), configuración OpenRouter compartida (`bias-lab-settings`/`openrouter_settings`) vía modal propio + botón en navbar, y retroalimentación IA incluida en vista previa y PDF final con fallback de estado cuando no hubo consulta).

4. Semana 11 — Herramientas de Seguridad y Privacidad en `informatica1/Semana11/` (`Semana11CiberSeguridad2.html` + `styles.css` + `app.js`, mismo patrón visual rojo ejecutivo que Semana 10, logo UMG en navbar vía `logo.png`): contenido Caps 8-9 parte 2 desde `Semana11_CiberSeguridad2.md` (autenticación 3 factores, MFA, ACL/RBAC, cifrado simétrico/asimétrico, firewall/VPN/antivirus, backups 3-2-1 + UPS/SAN, IDS + seguridad física, políticas/PUA + equilibrio usabilidad, PII/DPI + principios justos); 10 simuladores animados vanilla (ecosistema, fortaleza de contraseña, engranajes MFA, slider ACL vs RBAC, cifrador César/base64, mini-firewall, túnel VPN animado, checklist 3-2-1, radar IDS, balanza rigor, clasificador PII con trampa pedagógica, principios justos); ilustraciones blueprint extraídas de `Estrategia_de_Seguridad_Digital.pdf` (15 págs escaneadas) a `img/estr_p01–p15.jpg` (~2 MB, JPG q72/1400px) citadas como figuras; **ejercicio en parejas piloto+copiloto 40 min** (4 casos herramientas GT: A MFA cooperativa Q18,500, B RBAC retail Q9,800, C VPN/cifrado distribuidora Q22,000, D PII+backup clínica Q24,000, con fase de investigación 5–10 min y hallazgo exigido en justificación, **sin mentoría IA en esta página** (botón del mentor oculto por CSS, `consultarMentorIA` desactivada con `return` inmediato, `guardarCaso`/`avanzarCaso` sin consultas automáticas), PDF parejas `Apellidos_Carnets_Sem11.pdf`); presentación docente `semana11.pptx` (16 diapos, 1.5 MB, estilo oscuro ejecutivo #0B0F19 + banda roja #DC2626, Trebuchet MS + Calibri, 11 figuras `img/estr_p*.jpg` con alt, notas del orador en las 16, flujo conceptos→4 casos→página, construida con skill officecli-pptx); **tarea individual "La Silla del Gerente S11"** (Hotel Antigua, USB + backup falso, rescate Q28,000/48h, pérdida Q4,600/día, prevención Q300/mes+Q900+Q550=Q5,050, T.1/T.2/T.3 + Verificar + PDF `Apellido_Carnet_TareaSem11.pdf`); enlazada en `index_informatica.html` (hero + tarjeta destacada Semana 11).
